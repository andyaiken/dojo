...
export class AdventureScreen extends React.Component<Props, State> {
	...
	private getSidebar() {
		const scene = this.props.adventure.plot.scenes.find(s => s.id === this.state.selectedSceneID);
		if (scene) {
			const links = scene.links.map(link => {
				const target = this.props.adventure.plot.scenes.find(sc => sc.id === link.sceneID);
				if (target) {
					return (
						<Group key={link.id}>
				...
				<div>
					...
					<div className='section'>
						...
						<Dropdown
							...
							options={Verne.getPotentialLinks(this.props.adventure.plot, scene).map(s => ({ id: s.id, text: s.name || 'unnamed scene' }))}
							onSelect={id => this.props.addLink(scene, id)}
						/>
					</div>
					<hr/>
					<button className='disabled' onClick={() => null}>add an encounter</button>
					<ConfirmButton onConfirm={() => this.props.deleteScene(this.props.adventure.plot, scene)}>delete scene</ConfirmButton>
					<hr/>
			...
			return (
				<div>
					...
					<hr/>
					<button onClick={() => this.props.addScene(this.props.adventure.plot, null, null)}>add a scene</button>
				...
				<Row className='full-height'>
					...
					<Col span={18} className='scrollable'>
						<PlotPanel
							...
							addScene={(sceneBefore, sceneAfter) => this.props.addScene(this.props.adventure.plot, sceneBefore, sceneAfter)}
						/>
					</Col>
				</Row>
			);
